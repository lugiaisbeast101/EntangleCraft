...
import net.minecraft.entanglecraft.gui.EnumGui;
import cpw.mods.fml.common.network.IPacketHandler;
import cpw.mods.fml.common.network.Player;

public class ClientPacketHandler implements IPacketHandler{
	  
	  private void spawnParticleFromPacket(NetworkManager network, DataInputStream dataStream){
		  try{
	  ...
	  public void distanceHandlerUpdate(NetworkManager network, DataInputStream dataStream){
		  try{
	  ...
	  public void updateTileEntityField(NetworkManager network, DataInputStream dataStream)
	  {
		int tileEntityID = -1; // Using EnumGui indexes as ids, don't see why not
	    ...
	    if (xPos != null && yPos != null && zPos != null && te != null){ 	
		    if (tileEntityID == EnumGui.LambdaMiner.getIndex()) {
		    	...
		    	try{teLM.recieveUpdateFromServer(network, dataStream, field);}
		    	catch (Exception e){};
		    }
		    	...
		    	try{teGD.recieveUpdateFromServer(network, dataStream, field);}
		    	catch (Exception e){};
		    } 
	    }
	  } 
	  ...
	  public void playSoundFromPacket(NetworkManager network, DataInputStream dataStream) {
		  try{
	  ...
	  public void playTPSoundFromPacket(NetworkManager network, DataInputStream dataStream)
	  {
	    String playerName = "";
	    ...
	    if (shouldPlayPlayerSound) shouldPlayPlayerSound = EntangleCraft.getDistance(thisPlayerPos, playerPos) < 30.0D; 
	...
	@Override
	public void onPacketData(NetworkManager network, Packet250CustomPayload packet,
			Player player) {
		...
		if (!EntangleCraft.proxy.isServer){
		    ...
		    if (task == 0) playTPSoundFromPacket(network, dataStream); 
		    else if (task == 1) playSoundFromPacket(network, dataStream); 
		    else if (task == 2) updateTileEntityField(network, dataStream); 
		    else if (task == 3) distanceHandlerUpdate(network, dataStream);
		    else if (task == 4) spawnParticleFromPacket(network,dataStream);
		}
	}
}
