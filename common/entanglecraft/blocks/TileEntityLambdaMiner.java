...
public class TileEntityLambdaMiner extends TileEntity implements IInventory, ISidedInventory{
	...
	public void generateLayerStructure(){
		...
		int scanLeft = lMItemStacks[0] != null ? (lMItemStacks[0].itemID == new ItemStack(EntangleCraftItems.ItemTransmitter,1).itemID) ? lMItemStacks[0].stackSize : 0 : 0;
		int scanForward = lMItemStacks[1] != null ? (lMItemStacks[1].itemID == new ItemStack(EntangleCraftItems.ItemTransmitter,1).itemID) ? lMItemStacks[1].stackSize + 1 : 1 : 1;
		int scanRight = lMItemStacks[2] != null ? (lMItemStacks[2].itemID == new ItemStack(EntangleCraftItems.ItemTransmitter,1).itemID) ? lMItemStacks[2].stackSize : 0 : 0;
	...
	public void recieveUpdateFromServer(INetworkManager network, DataInputStream dataStream, String fieldName) throws IllegalArgumentException, IllegalAccessException{
	 ...
	 public void updateEntity()
	    {
	    	...
	    	super.updateEntity();
	    	if (hasStructureChanged()) generateLayerStructure();
	        boolean mined = false;
	        boolean isServer = EntangleCraft.proxy.isServer;
	        {
	            if (canSmelt())
	            {
	                ++this.processTime;
		        	...
		        	if (this.isMining == false && isServer){ // Updating the texture, though not implemented yet
	            ...
	            {
	            	if (isServer){
		                if (this.isMining) this.setIsMining(false);
	            	}
	            }
	        }
    ...
    private void smeltItem() {
    	if (EntangleCraft.proxy.isServer){
	    	...
	    	if (minerSoundPitch > 1F) minerSoundPitch = 1F;
	    	if (minerSoundPitch < 0.05F) minerSoundPitch = 0.05F;
			int x,y,z;
			x = this.blockCoords[0];
			y = layerToMine;
			z = this.blockCoords[2];
			...
			for (counter = 0 ; counter < reward.getSizeInventory(); counter ++){
				...
				if (itemStack != null){
					...
					EntityItem e = new EntityItem(this.worldObj,(double)this.blockCoords[0]+0.5,(double)this.blockCoords[1]+1.5,(double)this.blockCoords[2]+0.5, itemStack);
					e.dropItem(itemStack.itemID, itemStack.stackSize);
					}
					
				}
			}
			ServerPacketHandler.playSoundToClients(new double[] {(double)xCoord,(double)yCoord,(double)zCoord},this.worldObj.rand.nextFloat()*0.05F + 0.05F,minerSoundPitch, minerSound);
			ServerPacketHandler.playSoundToClients(new double[] {(double)xCoord, (double)this.layerToMine, (double)zCoord},this.worldObj.rand.nextFloat()*0.2F + 0.7F,minerSoundPitch,minerSound);
    	}
    }
    ...
    private int getInclusiveCost(int itemID){
    	...
    	else if (itemID == Block.oreIron.blockID || itemID == Block.oreCoal.blockID || itemID == Block.oreRedstone.blockID || itemID == Block.oreGold.blockID || itemID == Block.oreLapis.blockID){
    		cost = 256;
    	}
    	return cost;
    }
    ...
    private ItemStack getItemStackFromID(int blockID){
    	...
    	if(canMine(blockID)){
    		...
    		else if (blockID == Block.waterMoving.blockID || blockID == Block.waterStill.blockID || blockID == Block.lavaStill.blockID || blockID == Block.lavaMoving.blockID){
    			result = null;
    		}
    		...
    		else result = new ItemStack(Item.itemsList[blockID],1);
    	}
    ...
    private boolean canMine(int blockID){
    	...
    	if (this.filterInclusive && this.filteredIds != null){
    		canMine = false; // Assume you can't mine it for inclusive filtering
    ...
    private void addStackToInventory(InventoryBasic inv,ItemStack itemStack){
    	...
    	if (invContainsItem){
    		...
    		if (!overFlow) inv.setInventorySlotContents(counter,itemStack);
    		else {
    ...
    private void addStackToInventory(TileEntityChest inv,ItemStack itemStack){
    	...
    	if (!stackAdded){
    		...
    		if(inv.adjacentChestXNeg != null) addStackToInventory(inv.adjacentChestXNeg,itemStack);
    		else if(inv.adjacentChestXPos != null) addStackToInventory(inv.adjacentChestXPos,itemStack);
    		else if(inv.adjacentChestZNeg != null) addStackToInventory(inv.adjacentChestZNeg,itemStack);
    		else if(inv.adjacentChestZPosition != null) addStackToInventory(inv.adjacentChestZPosition,itemStack);
    		else {
    			EntityItem e = new EntityItem(this.worldObj,(double)this.blockCoords[0]+0.5,(double)this.blockCoords[1]+1.5,(double)this.blockCoords[2]+0.5, itemStack);
				e.dropItem(itemStack.itemID, itemStack.stackSize);
    		}
	...
	private boolean canSmelt() {
		...
		if (!this.worldObj.isRemote){ 
			if (DistanceHandler.getDistance(channel) < this.blockCost) canSmelt = false;
			if (this.layerToMine ==0) canSmelt = false;
