    ...
    {	
		...
		if (EntangleCraft.proxy.isServer){
			if (type == 3){
				...
				if (coords != null){
					ChunkCoordinates theCoords = thePlayer.verifyRespawnCoordinates(world, coords);
					...
					double expZ = thePlayer.posZ;
			          ...
			          ServerPacketHandler.playSoundToClients(new double[] {theCoords.posX+0.5,theCoords.posY, theCoords.posZ+0.5}, world.rand.nextFloat() * 0.2F + 0.5F, world.rand.nextFloat() * 0.2F + 0.8F, "tpScroll");
			          ServerPacketHandler.spawnParticleToClients(new double[] {theCoords.posX+0.5,theCoords.posY, theCoords.posZ+0.5},"largeexplode");
			          
			          double distance = EntangleCraft.getDistance(new double[] {expX,expY,expZ}, new double[] {thePlayer.posX,thePlayer.posY,thePlayer.posZ});
			          distance = distance < 30 ? 0 : distance < 96 ? (double)(int)Math.log(distance)*0.7 : distance < 256 ? (double)(int)Math.log(distance)*1.5: (double)(int)Math.log(distance)*2.5;
			          
			          if (distance != 0) world.createExplosion(thePlayer, expX, expY, expZ, (float)distance);
			          else ServerPacketHandler.spawnParticleToClients(new double[] {expX, expY, expZ},"largeexplode");
			          ServerPacketHandler.playSoundToClients(new double[] {expX,expY, expZ}, world.rand.nextFloat() * 0.2F + 0.5F, world.rand.nextFloat() * 0.2F + 0.8F, "tpScroll"); 
    ...
    {	
		if (EntangleCraft.proxy.isServer){
			...
			if (world.getBlockId(var12.blockX, var12.blockY, var12.blockZ) == Block.waterStill.blockID){
				...
				else shouldDamage = false;
			}
			int damageAmount = type == 3 ? 2 : 1;
			if (shouldDamage) thePlayer.getCurrentEquippedItem().damageItem(damageAmount, thePlayer);
